...
{
public:
    LocationSubscriber(std::string robotName): Node(robotName + "_location_subscriber")
    {
        subscription_ = this->create_subscription<common::msg::Location>(
                            "/sensor/"+ robotName + "_location", 5, std::bind(&LocationSubscriber::topic_callback, this,
                                    std::placeholders::_1));
    }

    const common::msg::Location& GetData()
    {
        return location_;
    }

private:
    void topic_callback(const common::msg::Location::SharedPtr msg)
    {
        location_ = *msg;
    }

    common::msg::Location location_;
    rclcpp::Subscription<common::msg::Location>::SharedPtr subscription_;
};